...
final class ScriptWindow: NSObject {
    ...
    var selectedTab: ScriptTab? {
        ...
        return ScriptTab(
            ...
            tab: tabHost(selectedController)?.activeTab)
    }
    ...
    func tabIsSelected(_ controller: BaseTerminalController, tab: TerminalTab? = nil) -> Bool {
        ...
        return tabHost(controller)?.activeTab === tab
    }

    /// A controller that owns its own tabs, rather than being one itself.
    ///
    /// With native tabbing a controller IS a tab, so this is nil and the tab
    /// list comes from AppKit's tab group instead.
    private func tabHost(_ controller: BaseTerminalController) -> TerminalController? {
        guard let controller = controller as? TerminalController,
    ...
    private var tabRefs: [TabRef] {
        controllers.flatMap { controller -> [TabRef] in
            guard let host = tabHost(controller) else {
                return [TabRef(controller: controller, tab: nil)]
            }
            return host.tabs.map { TabRef(controller: controller, tab: $0) }
        }
    }
