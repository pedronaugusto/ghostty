...
class TerminalTabBarAccessoryViewController: NSTitlebarAccessoryViewController {
    ...
    private func observeWindow() {
        guard windowCancellables.isEmpty, let window = view.window else { return }
        let center = NotificationCenter.default

        // The native bar dims its whole chrome when the window isn't key.
        center.publisher(for: NSWindow.didBecomeKeyNotification, object: window)
            .merge(with: center.publisher(for: NSWindow.didResignKeyNotification, object: window))
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in self?.syncKeyWindow() }
            .store(in: &windowCancellables)

        // Tabs share the window's width, so they have to be remeasured with it.
        center.publisher(for: NSWindow.didResizeNotification, object: window)
            .receive(on: DispatchQueue.main)
