...
class TerminalTabBarViewModel: ObservableObject {
    ...
    /// The same font as AppKit has it. The inline rename editor is a real
    /// `NSTextField` -- see `TerminalTabTitleField` -- so it needs this rather
    /// than the SwiftUI value.
    @Published var nsFont: NSFont?
    ...
    /// Hand keyboard focus back to the terminal once an in-place rename ends.
    ///
    /// The editor gives up first responder when it finishes, which leaves the
    /// window itself as first responder and keystrokes going nowhere until the
    /// user clicks. The native bar's editor has the same problem and
    /// `TerminalWindow` solves it the same way after `TabTitleEditor` finishes.
    func renameDidFinish() {
