...
class TransparentTitlebarTerminalWindow: TerminalWindow {
    ...
    private var nonNativeTabsCancellable: AnyCancellable?
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if let terminalController = self.terminalController,
               terminalController.usesNonNativeTabs {
                guard self.nonNativeTabsCancellable == nil else { return }
                self.nonNativeTabsCancellable = terminalController.$tabs
                    .map(\.count)
                    .removeDuplicates()
                    .dropFirst()
